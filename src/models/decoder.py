...
def decoder_state_dict(decoder: DecoderBackend) -> dict[str, np.ndarray]:
    """
    Serialize explicit decoder parameters into a flat numpy state dict.
    """
    if not hasattr(decoder, "params") or decoder.params is None:
        raise ValueError("Decoder has no params to serialize.")

    w = decoder.params

    if not isinstance(w, dict):
        raise TypeError(f"Decoder params must be a dict, got {type(w)!r}")

    required_top = {"embed", "mem_proj", "layers", "lm_head"}
    missing = required_top.difference(w.keys())
    if missing:
        raise ValueError(f"Missing decoder parameter keys: {sorted(missing)}")

    layers = w["layers"]
    if not isinstance(layers, list):
        raise TypeError("decoder.params['layers'] must be a list")

    state: dict[str, np.ndarray] = {
        "decoder.embed": np.ascontiguousarray(w["embed"], dtype=np.float32),
        "decoder.mem_proj": np.ascontiguousarray(w["mem_proj"], dtype=np.float32),
        "decoder.lm_head": np.ascontiguousarray(w["lm_head"], dtype=np.float32),
        "decoder.n_layers": np.asarray([len(layers)], dtype=np.int32),
    }

    required_layer = {"Wq", "Wk", "Wv", "Wo", "W1", "W2", "norm1", "norm2"}

    for i, layer in enumerate(layers):
        if not isinstance(layer, dict):
            raise TypeError(f"decoder.params['layers'][{i}] must be a dict")

        layer_missing = required_layer.difference(layer.keys())
        if layer_missing:
            raise ValueError(
                f"Missing decoder layer keys at layer {i}: {sorted(layer_missing)}"
            )

        state[f"decoder.layers.{i}.Wq"] = np.ascontiguousarray(layer["Wq"], dtype=np.float32)
        state[f"decoder.layers.{i}.Wk"] = np.ascontiguousarray(layer["Wk"], dtype=np.float32)
        state[f"decoder.layers.{i}.Wv"] = np.ascontiguousarray(layer["Wv"], dtype=np.float32)
        state[f"decoder.layers.{i}.Wo"] = np.ascontiguousarray(layer["Wo"], dtype=np.float32)
        state[f"decoder.layers.{i}.W1"] = np.ascontiguousarray(layer["W1"], dtype=np.float32)
        state[f"decoder.layers.{i}.W2"] = np.ascontiguousarray(layer["W2"], dtype=np.float32)
        state[f"decoder.layers.{i}.norm1"] = np.ascontiguousarray(layer["norm1"], dtype=np.float32)
        state[f"decoder.layers.{i}.norm2"] = np.ascontiguousarray(layer["norm2"], dtype=np.float32)

    return state


def load_decoder_state_dict(decoder: DecoderBackend, ckpt) -> None:
    """
    Load decoder parameters from a flat numpy checkpoint opened with np.load(...).
    """
    if not hasattr(decoder, "params") or decoder.params is None:
        raise ValueError("Decoder has no params to load into.")

    w = decoder.params
    n_layers_ckpt = int(ckpt["decoder.n_layers"][0])

    if n_layers_ckpt != len(w["layers"]):
        raise ValueError(
            f"Decoder layer mismatch: checkpoint={n_layers_ckpt}, model={len(w['layers'])}"
        )

    w["embed"][...] = ckpt["decoder.embed"]
    w["mem_proj"][...] = ckpt["decoder.mem_proj"]
    w["lm_head"][...] = ckpt["decoder.lm_head"]

    for i, layer in enumerate(w["layers"]):
        layer["Wq"][...] = ckpt[f"decoder.layers.{i}.Wq"]
        layer["Wk"][...] = ckpt[f"decoder.layers.{i}.Wk"]
        layer["Wv"][...] = ckpt[f"decoder.layers.{i}.Wv"]
        layer["Wo"][...] = ckpt[f"decoder.layers.{i}.Wo"]
        layer["W1"][...] = ckpt[f"decoder.layers.{i}.W1"]
        layer["W2"][...] = ckpt[f"decoder.layers.{i}.W2"]
        layer["norm1"][...] = ckpt[f"decoder.layers.{i}.norm1"]
        layer["norm2"][...] = ckpt[f"decoder.layers.{i}.norm2"]

    if hasattr(decoder, "_make_params_contiguous"):
        decoder._make_params_contiguous()